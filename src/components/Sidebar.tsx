...
export default function Sidebar() {
    ...
    return <div className={cn("fixed flex bottom-0 top-[100%] inset-x-0 max-h-screen md:static md:h-screen transition-all bg-background", expanded && "top-16")}>
        <div className="absolute bottom-[100%] inset-x-0 text-center">
            ...
            <div className={cn("items-center justify-center flex h-20 w-full bg-background overflow-hidden transition-all border-t", expanded && "h-0 border-t-0")}>
                <Button onClick={() => setExpanded(!expanded)} variant={currentSet == null ? "outline" : "default"}>
                    { currentSet == null ? "Select a set..." : <>
                        <div className="min-w-8 min-h-8">
                            <Image className="w-full h-auto block" src={currentSet.images.symbol} height={32} width={32} alt={currentSet.name} />
                        </div>
        ...
        <div className={cn("w-full md:max-w-64 flex flex-col h-full border-r border-t relative transition-all")}>
            ...
            <div className={cn("py-3 px-6 w-full shadow-md justify-center hidden md:flex")}><Image className="h-8 w-8" alt="Twinleaf Logo" {...Logo} /></div>
            <SetsMenu sets={[...setsQuery.data?.data ?? []].reverse()} className={cn("p-3 overflow-y-scroll flex-1 px-3")} collapseTextForMobile={!expanded} />
        </div>
    </div>;
}